...
function generateOtp() {
    return node_crypto_1.default.randomInt(100_000, 999_999).toString();
}
...
//   7. Return 200 OK
// ==============================================================================
app.post("/v1/accounts/sms/code", async (req, res) => {
    try {
...
// ==============================================================================
app.post("/v1/accounts/code", async (req, res) => {
    try {
        ...
        const isValid = codeBuffer.length === storedBuffer.length &&
            node_crypto_1.default.timingSafeEqual(codeBuffer, storedBuffer);
        if (!isValid) {
...
// ==============================================================================
app.post("/v1/verification/session", (_req, res) => {
...
app.post("/v1/verification/session/:sessionId/code", (_req, res) => {
...
// ==============================================================================
app.post("/v1/registration", (_req, res) => {
...
app.get("/v1/certificate/delivery", (_req, res) => {
...
app.put("/v2/keys", async (req, res) => {
    try {
        ...
        if (Array.isArray(preKeys) && preKeys.length > 0) {
            const preKeyStrings = preKeys.map((pk) => JSON.stringify(pk));
...
app.get("/v2/keys/:identifier/:deviceId?", async (req, res) => {
    try {
...
app.put("/v1/accounts/attributes", (_req, res) => {
    console.log("[mock] PUT /v1/accounts/attributes - Attributes updated");
    res.status(200).json({ status: "SUCCESS" });
});
app.post("/v1/directory/tokens", (_req, res) => {
...
app.post("/v1/directory/parewa", async (req, res) => {
...
app.get("/v1/accounts/username/:username", async (req, res) => {
    try {
...
app.get("/v1/accounts/number/:number", async (req, res) => {
    try {
...
app.get("/v1/profiles/:identifier", (req, res) => {
